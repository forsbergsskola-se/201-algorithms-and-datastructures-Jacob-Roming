...
public class Game : MonoBehaviour
{
    ...
    public IEnumerable<State> FindPath(State start, State end, int maxDepth)
    {
        Stack<State> path = new Stack<State>();
        path.Push(start);
        HashSet<State> visitedCells = new HashSet<State>();
        ...
        while (path.Count > 0)
        {
            ...
            {
                //if the path already contains the neighbour or if it already has been traversed, skip this loop
                if (visitedCells.Contains(neighbour)) continue; 
                path.Push(neighbour); //Otherwise, add it!
                visitedCells.Add(neighbour);//I hope my brain is right
                if (neighbour.Equals(end)) //If we find our goal, we are done
                {
                    return path;
                }
    ...
    public Dictionary<State,State> DijkstraMultiplayer(State start, State end, int playerNumber)
    {
        ...
        while (nodesToCheck.Count != 0)
        {
            ...
            foreach (var node in currentNode.GetSuccessors())
            {
                ...
                Debug.Log("The cells costPerPlayer is: " + cell.costPerPlayer[playerNumber]);
